...
@Component
public class FollowSsbnSuccessStrategy
        implements MissionSuccessStrategy {

    @Override
    public MissionType getMissionType() {
        return MissionType.FOLLOW_SSBN;
    }

    @Override
    public MissionOutcome resolve(SimulationResult result) {

        if (result == null || result.getDetectedContacts() == null) {
            return MissionOutcome.FAILURE;
        }

        boolean submarineDetected = result.getDetectedContacts().stream()
        ...
        boolean submarineClassified = result.getDetectedContacts().stream()
                .anyMatch(contact -> contact.getContactType() == ContactType.SUBMARINE
